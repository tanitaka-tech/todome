...
ボード上部または下部に **独立した「ライフログ」セクション** を設ける。
...
- **タスク計測とライフログ計測は全体として排他**（詳細は下記「計測の排他制御」参照）

---

## 計測の排他制御

**タスク計測とライフログ計測はシステム全体で同時に1つだけアクティブ**。
「食事しながら作業」のような二重計測は許可しない。

| 現在の状態 | ライフログ開始をタップ | タスク計測開始をタップ |
|---|---|---|
| 何も計測中でない | そのまま開始 | そのまま開始 |
| 他のライフログ計測中 | **確認ダイアログ** →「現在の◯◯を停止して△△を開始しますか？」 | 同左 |
...
- 日次/週次の振り返りドキュメントに「ライフログサマリー」セクションを自動生成
  ```
  ## ライフログ（今日）
  - 食事 45m × 3回
  - 風呂 25m ✅
  ...
  - SNS 40m ⚠ (上限30m)
  ```
- AI が振り返り対話で「遊びが上限を超えた日が多いですね。何が引き金になっていますか？」のような質問を投げられるよう、ライフログ集計を context に入れる

---

## 統計パネル連携

- `StatsPanel` に「ライフログ」タブを追加
...
### Step 3: 振り返り・統計連携
- [ ] 振り返りドキュメントにライフログサマリー自動生成
- [ ] `StatsPanel` にライフログタブ
- [ ] AI context にライフログ集計を投入

---

## Git同期

ライフログ関連のデータは既存の `github_sync.py` の同期対象に含める。

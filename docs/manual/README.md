...
トップページ上部の 4 本の Animated WebP(タスク完了時の褒めトースト / AI にタスク追加を依頼 / AI が振り返りを生成 / テーマ切替でアプリ全体の色味が変わる)は、専用の Playwright スペックで撮影 → `img2webp` で合成している。AI 応答は `window.WebSocket` をスタブ化した上で合成メッセージを注入する決定的なデモで、`ANTHROPIC_API_KEY` は不要。
...
生成物は `docs/manual/assets/animations/{celebration,chat-ai,retro-ai,theme-switch}.webp`。git にコミットして push する。

内部構成:

- `e2e/manual/ws-harness.js` — `window.WebSocket` を丸ごとラップし、`window.__wsInject()` から合成 WS メッセージを注入できるようにする Playwright init script
- `e2e/manual/animations.spec.ts` — 4 シナリオを再現しつつ 10fps でフレーム連写

...
- 目標との紐付け: content 先頭に [GOAL:<目標id>] を付ける (例: "[GOAL:abc12345] 企画書を作成")。紐付けを外す場合は [GOAL:] と書く。省略時は既存の紐付けを維持する
...
### 例: タスク追加と目標追加を同時に行う
TodoWrite の todos:
  [{"content":"企画書を作成","status":"pending"},{"content":"GOAL_ADD:{\\"name\\":\\"Q3売上目標\\",\\"memo\\":\\"前年比120%\\",\\"kpis\\":[{\\"name\\":\\"月間売上(万円)\\",\\"unit\\":\\"number\\",\\"targetValue\\":1000,\\"currentValue\\":0}],\\"deadline\\":\\"2026-09-30\\"}","status":"completed"}]
...
ノルマIDは末尾の「ノルマ一覧と今日の達成状況」セクションの \`(id: ...)\` から確認できる。

### ノルマ設定の更新
content を以下の形式にする (status は "completed"):
  QUOTA_UPDATE:<ノルマid>:{"name":"掃除","icon":"🧹","targetMinutes":20}
更新したいフィールドだけ含めればよい。

### ノルマ計測の開始
  QUOTA_LOG_START:<ノルマid>

### ノルマ計測の停止
  QUOTA_LOG_STOP

## タイムボックス操作 (TodoWrite の特殊エントリ)
タイムボックスの設定変更・計測開始/停止を行うには以下を使う。
アクティビティIDは末尾の「タイムボックス設定と今日の使用状況」セクションの \`(id: ...)\` から確認できる。

### タイムボックス設定の更新
content を以下の形式にする (status は "completed"):
  LIFE_UPDATE:<アクティビティid>:{"softLimitMinutes":60,"hardLimitMinutes":180,"category":"play","limitScope":"per_day"}
更新したいフィールドだけ含めればよい。
- category: "rest"(休息) / "play"(娯楽) / "routine"(日常) / "other"(その他)
- limitScope: "per_session"(セッション単位) / "per_day"(1日単位)

### タイムボックス計測の開始
  LIFE_LOG_START:<アクティビティid>

### タイムボックス計測の停止
  LIFE_LOG_STOP

## タイムスケジュール
末尾の「今日のタイムスケジュール」は、今日の時間軸上の実績を時系列で示したもの。
- タスク計測 (進行中/完了タスクの TimeLog と現在計測中のタイマー)
- ライフログ (食事・遊び・SNS 等、ユーザー自身が計測している生活時間)
- ノルマ計測 (掃除・運動 等の達成目標時間)
ユーザーの今日の過ごし方を踏まえて、休憩提案・時間配分の相談・次のアクション提案に活用すること。
「[計測中]」は現在進行中の計測で、未完了のまま継続している。
...
export function buildSystemPromptAppend(
  ...
  return TEMPLATE.replace("{today}", today)
    ...
    .replace("{timeline_context}", timelineContext);
}
